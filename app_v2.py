    ...
    with col1:
        ...
        if st.button("🔍 Consultar Agentes IA", type="primary", use_container_width=True):
            if not user_question.strip():
                ...
                # Processar com sistema multi-agente real
                with st.spinner("🤖 Consultando especialistas... Isso pode demorar alguns minutos"):
                    try:
                        # Importar e usar sistema real
                        from assistente_juridico_crew import AsistenteJuridicoAI
                        
                        # Inicializar sistema
                        assistente = AsistenteJuridicoAI()
                        
                        # Processar consulta
                        resposta_completa = assistente.processar_consulta(user_question)
                        
                        # Mostrar resultado
                        st.success("✅ Consulta processada com sucesso!")
                        st.markdown("### 💬 Resposta dos Agentes Especialistas:")
                        ...
                        st.markdown("### 🔧 Modo de Fallback - Informação Básica")
                        st.markdown(f"""
                        <div class="info-box">
                        <h4>🎯 Pergunta recebida:</h4>
                        <p><em>"{user_question}"</em></p>
                        
                        <h4>⚖️ Especialidade selecionada: {specialty}</h4>
                        
                        <h4>🤖 Status do erro:</h4>
                        <p>❌ Sistema multi-agente temporariamente indisponível</p>
                        <p>🔄 Erro: {str(e)}</p>
                        
                        <h4>📋 Recomendação:</h4>
                        <p>1. Verifique se as APIs estão configuradas no arquivo .env</p>
                        <p>2. Para esta consulta específica, recomendamos contactar um advogado especialista em {specialty.lower()}</p>
                        <p>3. Consulte a Ordem dos Advogados de Portugal para encontrar um profissional qualificado</p>
                        </div>
                        """, unsafe_allow_html=True)
